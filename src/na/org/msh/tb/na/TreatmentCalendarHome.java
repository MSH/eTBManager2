...
public class TreatmentCalendarHome {
	 ...
	 * @param end
	 */
	protected void createMonths(List<org.msh.tb.na.MonthInfo> lst, Date iniDate, Date endDate) {
	 ...
	 */
	public org.msh.tb.na.DayInfo getDay(Date dt) {
		Calendar c = Calendar.getInstance();
		c.setTime(dt);
		...
		org.msh.tb.na.MonthInfo mi = getMonth(month, year, day);
		if (mi != null) {
			org.msh.tb.na.DayInfo di = mi.getDays().get(day - 1);
			return di;
		}
	...
	protected void mountPlanned() {
		...
		while (!inidt.after(enddt)) {
			org.msh.tb.na.DayInfo dayInfo = getDay(inidt);
			if (dayInfo != null) {
	...
	public class PhaseInfo {
		private List<org.msh.tb.na.MonthInfo> months = new ArrayList<MonthInfo>();
