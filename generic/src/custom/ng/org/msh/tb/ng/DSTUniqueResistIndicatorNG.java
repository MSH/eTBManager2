...
public class DSTUniqueResistIndicatorNG extends Indicator2D {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 4166058241014286844L;
		...
		for (ResistancePattern pattern: resistancePatterns.getResultList()) {
			...
			if(tbCaseList.size()!=0){
				for(int k=0; k<tbCaseList.size();k++){
						if(tbCaseList.get(k).getPatientType().getKey().equalsIgnoreCase("PatientType.NEW")){
			...
			if(tbCaseList.size()!=0){
				 newPer = ((float)countNewPatients/(float)tbCaseList.size())*100;
			...
			String strOnlyRes = getMessage("manag.ind.dstprofile.onlyresistto");
			addValue(message[0],strOnlyRes+" "+ strPattern, new Float(countNewPatients));
			addValue(message[1],strOnlyRes+" "+ strPattern, new Float(newPer));
			addValue(message[2],strOnlyRes+" "+ strPattern, new Float(oldPat));
			addValue(message[3],strOnlyRes+" "+ strPattern, new Float(oldPer));
			addValue(message[4],strOnlyRes+" "+ strPattern, new Float(totalPatients));
			
		}
		
		//addValue( getMessages().get("manag.ind.resist.other"), total); //this is 1D
	}
